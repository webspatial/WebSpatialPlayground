...
export default function useTimer(time) {
  ...
  var timeRef = useRef(isCountDown ? typeof time === 'number' ? Date.now() + time : Number(time) : Number(time));
  var timeHandle = useRef();

  var _useState5 = useState(function () {
    return formatTime(timeRef.current, true);
  }),
  ...
  useUpdateEffect(function () {
    ...
    if (auto) {
      setisCounting(true);
      timeHandle.current && window.cancelAnimationFrame(timeHandle.current);
      timeHandle.current = window.requestAnimationFrame(function () {
        return showTime();
      });
    }
  ...
  var showTime = function showTime() {
    var formatedTime = formatTime(timeRef.current, !isCountDown);
  ...
  var timeMethods = useMemo(function () {
    ...
    var reset = function reset() {
      ...
      clearRef();
      var timeProperties = formatTime(Date.now() + Number(resetTime));
