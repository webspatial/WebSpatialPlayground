...
import CBoolean from './CBoolean'; // 提供 Request 类,供并行请求使用
...
var Request = /*#__PURE__*/function () {
  ...
  function Request(requestFn, options, onChangeState, initState) {
    ...
    this.count = 0;
    this.unmountedFlag = new CBoolean();
    this.that = this;
    ...
    this.state = {
      ...
      error: undefined,
      isPolling: false,
      unmountedFlag: this.unmountedFlag,
    ...
    var debounceThrottleOptions = options.debounceThrottleOptions,
    ...
    if (debounceInterval !== undefined) {
      this.debounceFn = debounceInterval ? debounce(this._run.bind(this.that), debounceInterval) : undefined;
    } else if (throttleInterval !== undefined) {
      this.throttleFn = throttleInterval ? throttle(this._run.bind(this.that), throttleInterval) : undefined;
    } else if (debounceThrottleOptions !== undefined) {
      var type = debounceThrottleOptions.type,
          interval = debounceThrottleOptions.interval,
          _debounceThrottleOpti = debounceThrottleOptions.leading,
          leading = _debounceThrottleOpti === void 0 ? type === 'debounce' ? false : true : _debounceThrottleOpti,
          _debounceThrottleOpti2 = debounceThrottleOptions.trailing,
          trailing = _debounceThrottleOpti2 === void 0 ? true : _debounceThrottleOpti2;

      if (type === 'debounce') {
        this.debounceFn = debounce(this._run.bind(this.that), interval, {
          leading: leading,
          trailing: trailing
        });
      } else if (type === 'throttle') {
        this.throttleFn = throttle(this._run.bind(this.that), interval, {
          leading: leading,
          trailing: trailing
        });
      }
  ...
  _createClass(Request, [{
    ...
    value: function _run() {
      ...
      return this.requestFn.apply(this, params).then(function (res) {
        if (_this.unmountedFlag.isTrue() || currentCount !== _this.count) {
          return;
        }
