...
var useBaseRequest = function useBaseRequest(requestFn) {
  var _options$debounceThro, _options$debounceThro2, _options$debounceThro3, _options$debounceThro4;
  ...
  var requestFnPersistCallback = usePersistCallback(requestFn);
  var onSuccessPersistCallback = usePersistCallback(onSuccess);
  var onErrorPersistCallback = usePersistCallback(onError);
  var formatResultCallback = formatResult ? usePersistCallback(formatResult) : undefined;
  var requestKeyCallback = usePersistCallback(requestKey);
  var onChangeState = useCallback(function (key, data) {
  ...
  var cacheStoreTypeRef = useRef(cacheStoreType);
  cacheStoreTypeRef.current = cacheStoreType;
  var readyRef = useRef(ready);
  readyRef.current = ready;
  var currentRequestKey = useRef(DEFAULT_KEY);

  var getNewRequestByCache = function getNewRequestByCache() {
    // 如果有 缓存，则从缓存中读数据
    if (cacheKeyRef.current) {
      var _ref = getCache(cacheKeyRef.current, cacheStoreTypeRef.current) || {},
          data = _ref.data,
      ...
      if (data && isAlive) {
        ...
        Object.keys(data.requests).forEach(function (key) {
          ...
          var newRequest = new Request(requestFnPersistCallback, _objectSpread(_objectSpread({}, options), {}, {
  ...
  var requestsRef = useRef(requests);
  requestsRef.current = requests;
  var readyMemoryParams = useRef(); // run method
  ...
  var run = useCallback(function () {
    ...
    if (!currentRequest) {
      var requestState = new Request(requestFnPersistCallback, _objectSpread(_objectSpread({}, options), {}, {
        onSuccess: onSuccessPersistCallback,
        onError: onErrorPersistCallback,
        formatResult: formatResultCallback
  ...
  useUpdateEffect(function () {
    if (cacheKeyRef.current) {
      setCache(cacheKeyRef.current, cacheTime, {
        requests: requests,
        currentRequestKey: currentRequestKey.current
      }, cacheStoreTypeRef.current);
    }
  }, [requests]); // 重置
  ...
  var refreshOptions = [options.debounceInterval, options.throttleInterval, (_options$debounceThro = options.debounceThrottleOptions) === null || _options$debounceThro === void 0 ? void 0 : _options$debounceThro.type, (_options$debounceThro2 = options.debounceThrottleOptions) === null || _options$debounceThro2 === void 0 ? void 0 : _options$debounceThro2.interval, (_options$debounceThro3 = options.debounceThrottleOptions) === null || _options$debounceThro3 === void 0 ? void 0 : _options$debounceThro3.leading, (_options$debounceThro4 = options.debounceThrottleOptions) === null || _options$debounceThro4 === void 0 ? void 0 : _options$debounceThro4.trailing, options.pollingInterval];
  useUpdateEffect(function () {
    Object.values(requests).forEach(function (request) {
      if (request.unmountedFlag.isTrue()) return;

      if (options.debounceInterval !== undefined) {
        request.refreshOptions({
          debounceInterval: options.debounceInterval,
          pollingInterval: options.pollingInterval
        });
      } else if (options.throttleInterval !== undefined) {
        request.refreshOptions({
        ...
        request.refreshOptions({
          debounceThrottleOptions: options.debounceThrottleOptions,
          pollingInterval: options.pollingInterval
        });
      }
    });
