...
import { getPreviewUrl, getKey, download } from './utils';
var initialState = {
  progress: 0,
  success: false,
  error: null
...
var useUpload = function useUpload(options) {
  ...
  var updateUploadItem = useCallback(function (key, newState) {
  ...
  var uploadHandler = useCallback(function (item, fileArr) {
    var key = item.key;
  ...
  }, [uploadRequest]); // 处理files

  var filesHandler = useCallback(_async(function (files, e) {
  ...
  var reUpload = useCallback(function (item) {
    var key = item.key;
    updateUploadItem(key, initialState);
    uploadHandler(item, [item.file]);
  }, []); // 移除下载项

  var removeUploadItem = useCallback(function (item) {
    var key = item.key;
    setUploadList(function (list) {
