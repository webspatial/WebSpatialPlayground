...
function getMergeValue(value, parseOptions, autoMergeUrlParams, isFirstMerged, autoMergeUrlParamsOptions) {
  var mergeValue = isObject(value) ? _objectSpread({}, value) : {};

  if (autoMergeUrlParams) {
    if (isFirstMerged) {
      mergeValue = Object.assign(mergeValue, _queryString.default.parse(window.location.search, parseOptions));
    } else {
      mergeValue = Object.assign(_queryString.default.parse(window.location.search, parseOptions), mergeValue);
    }
  } else {
    if (isFirstMerged && autoMergeUrlParamsOptions !== null && autoMergeUrlParamsOptions !== void 0 && autoMergeUrlParamsOptions.useUrlParamsOnFirst) {
      mergeValue = Object.assign(mergeValue, _queryString.default.parse(window.location.search, parseOptions));
    }
  }
...
function useUrlParams() {
  ...
  var _Object$assign = Object.assign({
    ...
    replaceUrl: true
      ...
      autoMergeUrlParamsOptions = _Object$assign.autoMergeUrlParamsOptions;

  var _useState = (0, _react.useState)(getMergeValue(initValue, parseOptions, autoMergeUrlParams, true, autoMergeUrlParamsOptions)),
  ...
  var initialValueRef = (0, _react.useRef)(value);
  var isFirstMerged = (0, _react.useRef)(true);
  var resetParams = (0, _react.useCallback)(function () {
    ...
    if (initial) {
      ...
      setValue(_queryString.default.parse(window.location.search, parseOptions));
    }
  }, []);
  var formatValue = (0, _react.useMemo)(function () {
  ...
  (0, _react.useEffect)(function () {
    try {
      ...
      if (isFirstMerged.current) {
        ...
        mergeValue = getMergeValue(formatValue, parseOptions, autoMergeUrlParams, isFirstMerged.current, autoMergeUrlParamsOptions);
      }

      var searchStr = _queryString.default.stringify((0, _lodash.default)(mergeValue, omitKeys), stringifyOptions);
      ...
      if (replaceUrl) {
        ...
        window.history.pushState(_objectSpread(_objectSpread({}, window.history.state), {}, {
          url: url,
          title: document.title
        }), document.title, url);
      } else {
