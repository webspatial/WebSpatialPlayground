import UTM from '../utm'
import { parseURL, parseUrlQuery } from '../../util/tool'
class Client {
  appid: number
  domain: string
  userAgent: string
  ...
  browser() {
    ...
    if (userAgent.indexOf('Edge') !== -1 || userAgent.indexOf('Edg') !== -1) {
      ...
      if (userAgent.indexOf('MQQBrowser') !== -1) {
        ...
        browser_version = userAgent.substring(versionOffset + 10, versionOffset + 25)
      }
  ...
  os() {
    ...
    const getHarmonyVersion = (ua) => {
      let version = getVersion(/OpenHarmony\s*([\d\.]+)/, ua)
      if (!version) {
        version = getVersion(/OpenHarmony\s*([\d\.]+)/, ua)
      }
      return version
    }
    ...
    switch (os_name) {
      ...
      case 'iOS':
        ...
        break;
      case 'chromeOS':
        const versionOffset = this.userAgent.indexOf('x86_64')
        os_version = this.userAgent.substring(versionOffset + 7, versionOffset + 16)
        break;
      case 'OpenHarmony':
        os_version = getHarmonyVersion(this.userAgent)
        os_name = 'OpenHarmony'
        break;
