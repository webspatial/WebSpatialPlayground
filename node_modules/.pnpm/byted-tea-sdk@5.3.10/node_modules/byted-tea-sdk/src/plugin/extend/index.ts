...
class PluginExtend {
  ...
  exist(pluginName: string, _object: any, src: string) {
    if (pluginObject[_object]) {
      ...
      console.log(`已有${pluginName}插件，避免重复加载~`)
    } else {
      this.loadPlugin(pluginName, src, () => {
        this.process(pluginName, pluginObject[_object])
        console.log(' %c %s %s %s', 'color: yellow; background-color: black;', '–', `load plugin:${pluginName} success`, '-')
      }, () => {
        console.log(' %c %s %s %s', 'color: red; background-color: yellow;', '–', `load plugin:${pluginName} error`, '-')
      })
    }
  }
