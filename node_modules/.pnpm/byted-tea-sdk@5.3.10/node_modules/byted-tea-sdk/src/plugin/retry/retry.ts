...
class Retry {
	...
	checkStorage() {
		try {
			...
			this.fetchData([loadData], success, () => {
				console.log('network error，compensate report fail');
			}, true);
		} catch (e) {
	...
	fetchData(data: any, success?: any, fail?: any, test?: boolean) {
		this.collect.requestManager.useRequest({
			...
			success,
			fail,
			app_key: test ? '566f58151b0ed37e' : ''
		})
	}
}

/**@@SCRIPT
const exportRetry = (collect: any, config: any) => {
...
try {
	...
	if (pluginObject) {
		pluginObject.LogStore = exportRetry
	}
} catch (e) {
	console.log(e)
}
