...
export default class EventCheck {
  ...
  checkEventParams(params: any): boolean {
    ...
    if (!Object.keys(_params).length) return true;
  ...
  calculate(name: string, type: string): boolean {
    const whiteList = type === 'event' ? this.eventNameWhiteList : this.paramsNameWhiteList;
    if (whiteList.indexOf(name) !== -1) return true;
    if (new RegExp('^\\$').test(name) || new RegExp('^__').test(name)) {
      console.warn(`${type === 'event' ? 'event' : 'params'} name: ${name} can not start with $ or __, pleace check;`);
      return false;
    }
    return true;
    // console.warn(`${type === 'event' ? 'event' : 'params'} name: ${name} is illegal, can not start with $ or __, pleace check;`);
    return false;
  }
}