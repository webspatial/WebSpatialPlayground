import { getText, getContainer, isAttrFilter, getAttributes, getElementText } from './dom'
...
export default function getElementData(event: any, element: HTMLElement, options: any, ignore?: IGNORE) {
  ...
  element = getContainer(element, options)
  ...
  const texts = getText(element, options)
  ...
  let elementObj = null
  if (window.TEAVisualEditor && window.TEAVisualEditor.getOriginXpath) {
  ...
  if (ignore && !ignore.text) {
    ...
    // 换一种实现方式
    elementData.element_title = element.getAttribute('data-tea-title') || element.getAttribute('title') || getElementText(element)
  }
  ...
  if (options.custom_attr) {
    try {
      if (isAttrFilter(element, options.custom_attr)) {
        const attrData = getAttributes(element, options.custom_attr)
        const attr = decodeURIComponent(attrData[options.custom_attr])
        const attrObj = JSON.parse(attr)
        if (Object.keys(attrObj)) {
          for (let key in attrObj) {
            elementData[key] = attrObj[key]
          }
        }
      }
    } catch (e) {
  ...
  const containerNoode = getContainer(element, options)
