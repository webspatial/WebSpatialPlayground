import Client from '../plugin/system'
import Storage from '../util/storage'
import { decodeUrl, handleQuery } from '../util/tool'
...
export default class ConfigManager {
  ...
  eventCheck: any
  enableClientTransport: boolean = false
  constructor(collect: any, initConfig: any) {
    ...
    this.envInfo = {
      ...
      header: {
        ...
        custom: commonInfo.network_type ? {
          network_type: commonInfo.network_type
        } : {},
    ...
    this.collect.on('route-change', (info) => {
      this.changeReferInfo(info.config);
    })
    this.setEnableClientTransport(this.initConfig.transport ? true : false);
  }
  initDomain() {
  ...
  getUrl(type: string) {
    ...
    let query = []
    query.push(`aid=${this.initConfig.app_id}&sdk_version=${SDK_VERSION}&device_platform=${this.envInfo.header.platform}`)
    if (this.initConfig.caller) {
      query.push(`caller=${this.initConfig.caller}`)
    }
    // if (this.initConfig.need_zip && type === 'event') {
    //   query.push('zip=1')
    // }
    return handleQuery(`${this.getDomain()}${report}`, query)
  }
  ...
  changeReferInfo(config: any) {
    const referConfig = config;
    this.envInfo.header.referrer = referConfig.referrer;
    let referHost = '';
    try {
      referHost = new URL(referConfig.referrer).host
    } catch (e) { }
    this.envInfo.header.referrer_host = referHost || this.envInfo.header.referrer_host;
  }

  setEnableClientTransport(enable: boolean) {
    this.enableClientTransport = enable;
  }

  getEnableClientTransport() {
    return this.enableClientTransport;
  }
        ...
        if (scope) {
          if (scope === 'user' || scope === 'header') {
            ...
            this.collect.emit(DebuggerMesssge.DEBUGGER_MESSAGE, {
              type: DebuggerMesssge.DEBUGGER_MESSAGE_SDK, info: `SDK 添加了自定义参数:${key}`, level: 'info', time: Date.now(), infoType: 'sdk', secType: 'HEADER', common: 'custom'
            })
          }
          ...
          this.collect.emit(DebuggerMesssge.DEBUGGER_MESSAGE, {
            type: DebuggerMesssge.DEBUGGER_MESSAGE_SDK, info: `SDK 修改了公共参数:${key}`, level: 'info', time: Date.now(), infoType: 'sdk', secType: 'USER', common: key
          })
        } else if (Object.hasOwnProperty.call(this.envInfo.header, key)) {
          this.envInfo.header[key] = info[key]
          this.collect.emit(DebuggerMesssge.DEBUGGER_MESSAGE, {
            type: DebuggerMesssge.DEBUGGER_MESSAGE_SDK, info: `SDK 修改了公共参数:${key}`, level: 'info', time: Date.now(), infoType: 'sdk', secType: 'HEADER', common: key
          })
        } else {
          this.envInfo.header.custom[key] = info[key]
          this.collect.emit(DebuggerMesssge.DEBUGGER_MESSAGE, { type: DebuggerMesssge.DEBUGGER_MESSAGE_SDK, info: `SDK 添加了自定义参数:${key}`, level: 'info', time: Date.now(), infoType: 'sdk', secType: 'HEADER', common: 'custom' })
        }
      }
    })
  }
